...
export function PriceReference({name,items,onAdopt}){
 ...
 const search=async()=>{
  ...
  try{const response=await fetch(`${import.meta.env.VITE_AI_ENDPOINT.replace(/\/$/,'')}/price-reference`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name}),signal:request.signal});
 ...
 return <details className="price-reference"><summary>不知道卖多少？看看参考价</summary>
  <p className="fine-print">{result?'公开网页报价参考，可能不是同一型号或成色；不代表已成交。':'本站目前是演示市集，下列为同类示例挂牌价，不是实时市场价。'}</p>
  {samples.length>0?<ul>{samples.slice(0,6).map((s,i)=><li key={s.id||s.url||i}><div><b>{s.name||s.title}</b><small>{s.source||'公开网页报价'}{s.publishedAt?` · 页面日期 ${s.publishedAt}`:''}</small>{s.url&&/^https?:\/\//.test(s.url)&&<a href={s.url} target="_blank" rel="noreferrer">查看价格来源 ↗</a>}</div><strong>¥{s.price}</strong></li>)}</ul>:<p className="fine-print">还没有可比较的同类价格，补全物品名称或型号再试。</p>}
