...
export async function analyzePhoto(image,signal,note=''){
 ...
 const r=await fetch(`${endpoint.replace(/\/$/,'')}/listing-draft`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image,note:note.slice(0,1000)}),signal});
