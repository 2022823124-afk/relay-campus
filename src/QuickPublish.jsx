...
export function Publish({Modal,close,submit,school,items=[],initialMode='sale'}){
 ...
 const prepare=async(useAI)=>{
  ...
  if(mounted.current&&controller.current===request){
   setD(v=>({...v,name:draft.name,description:draft.description,category:draft.category||'',price:mode==='free'?'0':draft.price,confirmed:false}));
 ...
 const finish=async()=>{
  const finalDescription=answerDescription(d.description,answers)+(d.price==='0'&&pickupNote.trim()?'\n领取说明：'+pickupNote.trim():'');
  if(finalDescription.length>1500)return setError('描述和领取说明合计请控制在 1500 字以内。');
 ...
 return <Modal title={mode==='free'?'送一件好物，接一段故事':'拍张照，让好物接力'} close={close} back={step===2?()=>{setStep(1);set('confirmed',false)}:close}>
  ...
  {step===1?<>
   <div className="publish-modes" role="group" aria-label="发布方式"><button aria-pressed={mode==='sale'} onClick={()=>{setMode('sale');set('price','')}}>卖闲置</button><button aria-pressed={mode==='free'} onClick={()=>{setMode('free');set('price','0')}}>免费接力 · 送给需要的人</button></div><p className="modal-lead">{mode==='free'?'拍下真实状况，说说它的故事。免费接力不需要定价。':'不用想怎么填表。先给好物拍张照。'}</p>
   ...
   <label className="field">随口说一句（可选）<textarea maxLength={1000} rows={2} disabled={busy} value={note} onChange={e=>setNote(e.target.value)} placeholder={mode==='free'?'例如：毕业带不走的花盆，免费送，周五傍晚可到校门交接。':'例如：台灯，开关正常，底座有划痕，想卖 30 元。'}/></label>
   {mode!=='free'&&extractOffer(note)!==''&&<p className="offer-detected" role="status">已识别你的报价 <b>¥{extractOffer(note)}</b> · 下一步自动填入，可修改</p>}
   ...
   {d.price==='0'?<label className="field free-pickup">领取说明（可选）<textarea maxLength={300} rows={3} value={pickupNote} onChange={e=>{setPickupNote(e.target.value);set('confirmed',false)}} placeholder="例如：希望送给会继续使用的同学，周五傍晚可到校门交接。"/><small>写可领取时间和需要确认的事项；请勿公开电话或宿舍地址。</small></label>:<PriceReference name={d.name} category={d.category} items={items} currentPrice={d.price} onAdopt={value=>set('price',value)}/>}</div>
   ...
   <div className="quick-summary"><b>提交前核对</b><p>{d.price!==''?d.price==='0'?'免费赠送 · ¥0':`报价 ¥${d.price}`:'报价待填写'} · {validMeeting(d.school,d.gate)?meetingLabel(d.school,d.gate):'交接点待选择'}</p><p>历史来源：{d.history==='upload'?'上传凭证':d.history==='statement'?'物主陈述':'未知'}{Object.values(answers).filter(Boolean).length>0?'；补充回答将加入物品描述。':''}</p></div>
   ...
   <div className="modal-actions"><button className="outline-button" disabled={busy} onClick={()=>{setStep(1);set('confirmed',false);setError('')}}><ArrowLeft size={17}/>上一步：照片与原话</button><button className="btn purple" disabled={busy||!d.confirmed} onClick={finish}>确认并提交<CheckCircle size={18}/></button></div>
  </>}
