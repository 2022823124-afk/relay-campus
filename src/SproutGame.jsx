...
export function SproutGame({garden,setGarden,drafts,items,onPublish}){
 ...
 const trial=()=>{if(care.trial){cheer('那盆花已经有新主人啦，故事还在继续。','happy');return}setCare(c=>({...c,trial:true,love:c.love+3}));cheer('花盆找到新主人啦！我也长大了一点。','gift')};
 ...
 {(panel==='stories'||panel==='coupons')&&<section className="sprout-records"><RelayGarden key={recordsTab} initialTab={recordsTab} compact garden={garden} setGarden={setGarden} drafts={drafts} items={items}/></section>}
 {panel==='help'&&<section className="sprout-sheet"><h4>小芽的生活，很简单</h4><p>点阳光、浇水、摸摸小芽，攒亲密值解锁装扮。每天可收 3 份阳光，浇水 +2、第一次摸摸 +1；不来打卡也不会枯萎。</p><p>这些互动只用于养成，不兑换曝光。完成平台内的双方确认演示后，接力成长和演示券才会进入“相册 / 我的券”。免费接力 40 点、2 张券；普通交易 20 点、1 张券。同一物品只奖励一次。</p><p>当前是本机游戏，进度只保存在这个浏览器。曝光券仍是本机推荐演示，不产生跨用户推流。</p></section>}
